			      ...
			      if (isConfirm) {
			        swal({
			          ...
			          icon: 'success'
			        	...
			        	$("form[name=frmList]")
						.prop("action","<c:url value='/admin/post/pdeleteMulti.do'/>");
			      ...
			      if (isConfirm) {
			        swal({
			          ...
			          icon: 'success'
			        	...
			        	$("form[name=frmList]")
						.prop("action","<c:url value='/admin/post/presMulti.do'/>");
						$("form[name=frmList]").submit();
			        });
			      } else {
			        swal("Cancelled", "취소되었습니다:)", "error");
...
function detailView(no) {
	document.detailFrm.pfNo.value=no;
	detailViewSubmit();
}
function detailViewSubmit(){
	...
	frm.action = "<c:url value='/admin/post/portDetail.do'/>";
	frm.target = "viewer";
	frm.method = "post";
	frm.submit();
}
...
<div class="title" style="width:1400px;">PORTFOLIO 관리 Page</div>
...
<c:if test="${empty param.searchKeyword}">
	<p>총 ${pageVo.totalRecord}건의 PORTFOLIO가 조회되었습니다. </p> 
</c:if>
</div>
<hr>
<br>
...
<div class="divA">
	...
	<form name="frmSearch" method="post" action='<c:url value="/admin/post/portfolio.do"/>'>
	...
	<div class="divSearch" style="width:270px;float:right;">
	   	<form name="frmSearch" method="post" 
	   		action='<c:url value="/admin/post/portfolio.do"/>'>
	        <div>
	        		<button type="submit" style="float:right;" class="mBtn2">검색</button>
					<input type="text" name="searchKeyword"
						placeholder="작가명, 포트폴리오 타이틀로 검색" style="width:220px; float:left;"> 
...
<table class="box2" style="width:1400px;margin-left:30px;">
	...
	<thead style="text-align:center;">
	  <tr>
	    ...
	    <th scope="col" id="ttlt">제목</th>
	    <th scope="col" id="ttlt">작성자</th>
	    <th scope="col" id="ttlt">작성일</th>
	    <th scope="col" id="ttlt">멤버쉽 유효</th>
	  </tr>
	</thead> 
	...
	<tbody>  
		...
		<c:if test="${!empty pList }">
		  	...
		  	<c:forEach var="map" items="${pList}">
	  			<tr >
	  				<td><input type="checkbox" name="pdItems[${idx}].pfNo"
				value="${map['PFNO']}" ></td>
					<td id="ttl">${map['PFNO']}</td>
					<td id="ttl"> 
					<c:choose>
					...
					<td id="ttl" style="font-weight:bold;" > 
						...
						<a href="#" onclick="detailView('${map['PFNO']}')">
						...
						<c:choose>
							...
							<c:otherwise>				
									<!-- 내용 일부만 보여주기 -->
									<%-- <c:if test="${fn:length(${map['PFTITLE']})>18}">	
										${fn:substring(${map['PFTITLE']}, 0, 18)}...
									</c:if>
									<c:if test="${fn:length(${map['PFTITLE']})<=18}"> --%>
										${map['PFTITLE']}
									<%-- </c:if>	 --%>							
					...
					<td id="ttl"> ${map['SNAME']} </td>
					<td id="ttl">
					
					<fmt:formatDate value="${map['REGDATE']}" pattern="yyyy-MM-dd"/>  </td>
					<td id="ttl"> ${map['AUTHORITY']} </td>	
				</tr>
