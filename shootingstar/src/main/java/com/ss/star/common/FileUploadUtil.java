...
public class FileUploadUtil {
	...
	public List<Map<String, Object>> fileUpload(HttpServletRequest request, int pathFlag)
		...
		/*
		 * //파일 정보를 저장할 컬렉션 List<Map<String, Object>> list = new ArrayList<>();
		 * 
		 * Iterator<String> iter = fileMap.keySet().iterator(); while(iter.hasNext()) {
		 * String key =iter.next(); MultipartFile tempFile = fileMap.get(key); //업로드된
		 * 파일이 있으면 if(!tempFile.isEmpty()) { //업로드된 파일 정보 구하기 String
		 * originalFileName=tempFile.getOriginalFilename(); long
		 * fileSize=tempFile.getSize(); String
		 * fileName=getUniqueFileName(originalFileName);
		 * 
		 * //업로드 처리하기 File file = new File(getUploadPath(request, pathFlag), fileName);
		 * tempFile.transferTo(file);
		 * 
		 * //파일 정보를 map에 저장 Map<String, Object> map = new HashMap<>();
		 * map.put("originalFileName", originalFileName); map.put("fileSize", fileSize);
		 * map.put("fileName", fileName);
		 * 
		 * //map을 list에 저장 list.add(map); } }//while
		 */
		return list;
	}
